...
static void* float_add(const void* a, const void* b) {
    double* result = malloc(sizeof(double));
    *result = *(const double*)a + *(const double*)b;
    return result;
}

static void* float_multiply(const void* a, const void* b) {
    double* result = malloc(sizeof(double));
    *result = *(const double*)a * *(const double*)b;
    return result;
}

static void* float_scalar_multiply(const void* scalar, const void* element) {
    return float_multiply(scalar, element);
}

static void* float_zero() {
    double* zero = malloc(sizeof(double));
    *zero = 0.0;
    return zero;
}

static void float_free_element(void* element) {
    free(element);
}

static void float_print(FILE* file, const void* element) {
    fprintf(file, "%.2f", *(const double*)element);
}

static void* float_from_str(const char* str) {
    double* value = malloc(sizeof(double));
    sscanf(str, "%lf", value);
    return value;
}

static void* float_copy(const void* element) {
