...
void* complex_scalar_multiply(const void* scalar, const void* element) {
...
void complex_free_element(void* element) {
    free(element);
}

void complex_print(FILE* file, const void* element) {
    const Complex* c = element;
    fprintf(file, "%.2f + %.2fi", c->real, c->imag);
}

void* complex_from_str(const char* str) {
...
void* complex_copy(const void* element) {
